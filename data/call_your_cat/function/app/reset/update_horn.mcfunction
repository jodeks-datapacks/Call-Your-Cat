$tag @e[predicate=call_your_cat:callable,tag=jodek.can_be_called,nbt={UUID:$(UUID_from_item)}] remove jodek.can_be_called

$data remove storage call_your_cat:root data.cat_Pos."$(UUID_from_item).x"

$data remove storage call_your_cat:root data.cat_Pos."$(UUID_from_item).z"
